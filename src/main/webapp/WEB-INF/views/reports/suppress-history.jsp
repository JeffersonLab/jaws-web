...
<c:set var="title" value="Suppress History"/>
<t:reports-page title="${title}">  
    <jsp:attribute name="stylesheets">
        <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/v${initParam.releaseNumber}/css/suppress-history.css"/>
    </jsp:attribute>
    <jsp:attribute name="scripts">
        <script type="text/javascript" src="${pageContext.request.contextPath}/resources/v${initParam.releaseNumber}/js/suppress-history.js"></script>
    </jsp:attribute>
    <jsp:body>
        <section>
                                    ...
                                    <table class="data-table inner-table">
                                        ...
                                        <c:forEach items="${notificationList}" var="notification">
                                            <tr data-id="${notification.alarm.alarmId}" data-action-id="${notification.alarm.action.actionId}" data-location-id-csv="${notification.alarm.locationIdCsv}" data-device="${notification.alarm.device}" data-screen-command="${notification.alarm.screenCommand}" data-masked-by="${notification.alarm.maskedBy}" data-pv="${notification.alarm.pv}">
                                                <td>
                                                    <fmt:formatDate value="${notification.suppressedHistoryPK.suppressedStart}" pattern="dd-MMM-yyyy HH:mm:ss"/>
                                                    <c:if test="${notification.suppressedEnd ne null}">
                                                        <fmt:formatDate value="${notification.suppressedEnd}" pattern="dd-MMM-yyyy HH:mm:ss" var="end"/>
                                                        <c:set var="duration" value="${notification.suppressedEnd.time - notification.suppressedHistoryPK.suppressedStart.time}"/>
                                                        <div title="${end}">(<c:out value="${jaws:millisToHumanReadable(duration)}"/>)</div>
                                                    </c:if>
                                                </td>
                                                <td>
                                                    <c:url value="/inventory/alarm-detail" var="url">
                                                        <c:param name="name" value="${notification.suppressedHistoryPK.name}"/>
                                                    </c:url>
                                                    <a title="Alarm Information" class="dialog-ready"
                                                       data-dialog-title="Alarm Information: ${fn:escapeXml(notification.suppressedHistoryPK.name)}"
                                                       href="${url}"><c:out
                                                            value="${notification.suppressedHistoryPK.name}"/></a>
                                                </td>
                                                <td><c:out value="${notification.alarm.action.priority.name}"/></td>
                                                <td><c:out value="${notification.activationType}"/></td>
                                                <td><c:out value="${notification.suppressedWith}"/></td>
                                                <td>
                                                    <c:choose>
