...
public class BatchNotificationService {
  ...
  public void oracleMergeHistoryBatchWithUniqueNames(
    ...
    // We must maintain insert and update order, event by event, so must parse batches to contain
    // unique alarms

    while (!clone.isEmpty()) {
      Map<String, EventSourceRecord<String, EffectiveNotification>> batch = new LinkedHashMap<>();
      ListIterator<EventSourceRecord<String, EffectiveNotification>> iterator =
          clone.listIterator();
