 ...
 */
@WebServlet(
    name = "AlarmDetailController",
    urlPatterns = {"/inventory/alarm-detail"})
public class AlarmDetailController extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (alarm != null) {
      // We couldn't get @OneToOne to work in Hibernate 5.3 so we manually add.  This has nice
      // benefit of only
      // loading for detail page and avoiding loading on list page, but it loads here as extra query
      // instead of join
      // so not great.
      //
      // We did try adding @PrimaryKeyJoinColumn to Alarm and @MapsId with separate Id field to
    ...
    request
        .getRequestDispatcher("/WEB-INF/views/inventory/alarms/alarm-detail.jsp")
